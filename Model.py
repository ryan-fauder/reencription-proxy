...
class Token(db.Model):
    ...
    def __init__(self, doctor_id, public_key_patient, public_key_doctor, key_frag, link):
        self.doctor_id = doctor_id 
        self.public_key_patient = public_key_patient 
        self.public_key_doctor = public_key_doctor 
        self.key_frag = key_frag 
        self.link = link 
        self.token = jwt.encode({'doctor_id': doctor_id, 'exp': datetime.utcnow() + timedelta(minutes=30)}, os.environ['SECRET_KEY'])
